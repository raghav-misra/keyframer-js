...
(elementQuery, transitionData, changedAttributes = {}, callback=()=>{}) => {
    document.querySelector(elementQuery).style.transition = transitionData;
    keyframer.setStyles(elementQuery, changedAttributes, callback);
};

// Create Transition On "EVENT":
keyframer.createCSSTransitionOn = 
(event, elementQuery, transitionData, changedAttributes={}, eventReceiver=null, callback=()=>{}) => {
    if(!isNotNullUndefinedOrNaN(eventReceiver)) eventReceiver = elementQuery;
    keyframer.revokeEvent(elementQuery, event);
    createKeyframerEvent(
        eventReceiver, elementQuery, event,
        ()=>{ keyframer.createCSSTransition(elementQuery, transitionData, changedAttributes, callback); }
    );      
...
(keyEvent, elementQuery, transitionData, allowedKeys, changedAttributes={}, eventReceiver=null, callback=()=>{}) => {
    if(!isNotNullUndefinedOrNaN(eventReceiver)) eventReceiver = elementQuery;
    keyframer.revokeEvent(elementQuery, "key"+keyEvent);
    createKeyframerEvent(
        eventReceiver, elementQuery, "key"+keyEvent,
